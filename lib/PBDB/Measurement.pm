...
sub getMeasurementTable {
    ...
    foreach my $row (@measurements) {
        if (!$seen_specimens{$row->{'specimen_no'}}) {
            ...
            $seen_ref{PBDB::Reference::formatShortRef($row)}++;
            $seen_part_ref{$row->{'specimen_part'}}{PBDB::Reference::formatShortRef($row)}++;
        }
...
# 		my %eco_lookup;
# 		for my $p ( @parents )	{
# 			my %temp_lookup = %{PBDB::Ecology::fastEcologyLookup($dbt,'life_habit',$p->{lft},$p->{rgt})};
...
# 	if ( @subspp )
# 	{
# 	    $sql = "SELECT t.taxon_no as child_no, a.taxon_no as parent_no
# 		FROM $TAXA_TREE_CACHE as t join opinions as o using (opinion_no)
# 			join authorities as a on a.taxon_no = o.parent_spelling_no
# 		WHERE t.taxon_no IN (".join(',',@subspp).") AND a.taxon_rank='species'";
...
# 	my $outfile = PBDB::PBDBUtil::getFilename($person)."_raw_measurements.txt";
# 	my $outfile2 = PBDB::PBDBUtil::getFilename($person)."_average_measurements.txt";
# 	my $outfile3 = PBDB::PBDBUtil::getFilename($person)."_measurement_table.txt";
