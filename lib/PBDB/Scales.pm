...
sub processEditScaleForm	{
	...
	PBDB::TimeLookup::buildLookupTable($dbt);
...
sub displayInterval	{
	...
	my $general_html = "";
	if ($itv->{'base_age'} =~ /\d/)	{
		my $lower = PBDB::TimeLookup::printBoundary($itv->{'base_age'});
		...
		my $upper = PBDB::TimeLookup::printBoundary($itv->{'top_age'}); 
...
sub submitSearchInterval {
    ...
    if (!$row) {
        $eml = PBDB::PBDBUtil::stripTags($eml);
        $name = PBDB::PBDBUtil::stripTags($name);
