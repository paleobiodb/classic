...
sub displayLoadImageForm{
    ...
    my @results = PBDB::TaxonInfo::getTaxa($dbt,{'taxon_no'=>$q->param('taxon_no')});
    if (@results) {
...
sub gallery	{
	...
	if ( ! PBDB::PBDBUtil::checkForBot() && (  $q->request_method() eq 'POST' || $q->param('is_real_user') || $s->isDBMember() ) )	{
		PBDB::logRequest($s,$q);
	} else	{
	...
	my @matches = PBDB::TaxonInfo::getTaxonNos($dbt,$taxon_name,'','',$q->param('author'),$q->param('pubyr'),$q->param('type_body_part'),$q->param('preservation'));
	my @taxon_nos = PBDB::TaxaCache::getChildren($dbt,$matches[0]);
...
sub displayImage {
    ...
    if (!$row) {
        ...
        my $ss = PBDB::TaxaCache::getSeniorSynonym($dbt,$row->{'taxon_no'});
        ...
        if ( $row->{reference_no} > 0 ) {
            ...
            my $ref_string = PBDB::Reference::formatShortRef($ref,'link_id'=>1);  
