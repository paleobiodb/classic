...
if ($ENV{'REMOTE_ADDR'} =~ /^188.186.181|^123.8.131.44|^81.110.187.44/){exit;}
...
sub displayDownloadGenerator {
	...
	$vars{'authorizer_me'} = $s->get("authorizer_reversed");
	$vars{'enterer_me'} = $s->get("authorizer_reversed");
	$vars{'data_url'} = $Constants::DATA_URL;
...
sub processCollectionsSearchForAdd	{
    ...
    for my $dr (@dataRows)	{
        ...
        #  the collection
        my $distance = 111 * CollectionEntry::GCD($mylat,$lat,abs($mylng-$lng));
        if ( $distance < 100 )	{
...
#        : ($goal eq 'image')      ? 'displayLoadImageForm'
    ...
    #     Images::displayLoadImageForm($dbt,$q,$s); 
        ...
        $q->param('taxon_no'=>$results[0]->{'taxon_no'});
        EcologyEntry::populateEcologyForm($dbt, $hbo, $q, $s, $WRITE_URL);
...
sub startProcessEcologyForm	{
    print $hbo->stdIncludes($PAGE_TOP);
	EcologyEntry::processEcologyForm($dbt, $q, $s, $WRITE_URL);
    print $hbo->stdIncludes($PAGE_BOTTOM);
}
## END Ecology stuff
##############
...
sub deleteSpecimen {
    my $err_msg = MeasurementEntry::deleteSpecimen($dbt, $hbo, $q, $s);
    if ( $err_msg )
    {
	$s->{error_msg} = $err_msg;
    }
