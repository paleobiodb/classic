...
package PBDB::Neptune;

use strict;
use DBI;
use PBDB::Debug;
use PBDB::Constants qw($WRITE_URL $READ_URL $HTML_DIR);
use PBDB::PBDBUtil;
use PBDB::Download;
...
sub setupOutput {
    ...
    PBDB::PBDBUtil::autoCreateDir($OUT_FILE_DIR);
...
sub displayNeptuneDownloadResults {
    ...
    if ($q->param('data_source') =~ /both/) {
        ...
        my $d = new PBDB::Download($dbt,$q2,$s,$hbo);
    ...
    if ( $q->param('yourname') ne "" )  {
        $yourname = "&yourname=".PBDB::PBDBUtil::getFilename($q->param('yourname')); 
    }
    my $ext = "csv";
    if ($q->param("output_format") eq 'tab') {
        $ext = "tab";
...
sub queryNeptuneDB {
    ...
    if (@errors) {
        print PBDB::Debug::printErrors(\@errors);
        exit;
    }
