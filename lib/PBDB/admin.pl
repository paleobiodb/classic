...
my $roles = $s->get('roles');

unless ($roles =~ /officer/) {
    print "Not logged in";
    die;
}
&$action;
...
sub insertRecord {
	...
	foreach my $fieldName (@fieldNames) {
		...
		if ( defined $q->param($fieldName) ) {
			...
			# Add quotes if this is character data (or if it is numeric since mysql doesn't care)
	...
	$sql =	"INSERT INTO $table (" . join(',', @fields) . ") ".
			" VALUES (" . join(',', @values) . ")";
	#$sql =~ s/\s+/ /gms; # Not necessary since we are quoting all values
	dbg ( "$sql<HR>" );
