package PBDB::FossilRecord;

use PBDB::Taxon;
use PBDB::TimeLookup;
use Data::Dumper;
use PBDB::Collection;  # not used
use PBDB::Reference;
use PBDB::TaxaCache;
use PBDB::Ecology;  # not used?
use PBDB::Images;  # not used
use PBDB::Measurement;  # not used
...
sub submitSearchTaxaForm {
    ...
    if ($q->param('taxon_no')) {
        ...
        my ($eml_max,$max) = PBDB::TimeLookup::splitInterval($q->param('max_interval_name'));
        my ($eml_min,$min) = PBDB::TimeLookup::splitInterval($q->param('min_interval_name'));
...
#                print $sql;
        ...
        my @results = PBDB::TaxonInfo::getTaxa($dbt,$options,['taxon_no','taxon_rank','taxon_name','common_name','author1last','author2last','otherauthors','pubyr','pages','figures','comments']);   
...
sub displayTaxon {
    ...
    my $orig_taxon_no = PBDB::TaxonInfo::getOriginalCombination($dbt,$taxon_no);
    $taxon_no = PBDB::TaxonInfo::getSeniorSynonym($dbt,$orig_taxon_no);
    # This actually gets the most correct name
    my $taxon = PBDB::TaxonInfo::getMostRecentSpelling($dbt,$taxon_no);
    ...
    if ($taxon_no && $common_name !~ /[A-Za-z]/) {
        my $orig_ss = PBDB::TaxonInfo::getOriginalCombination($dbt,$taxon_no);
        my $mrpo = PBDB::TaxonInfo::getMostRecentClassification($dbt,$orig_ss);
        ...
        if ($taxon_rank !~ /genus|species/) {
            %disused = %{PBDB::TaxonInfo::disusedNames($dbt,$ss_no)};
        }
...
';
	...
	if($modules{1}) {
		...
		print PBDB::TaxonInfo::displayTaxonClassification($dbt, $taxon_no, $taxon_name,$is_real_user);
        ...
        print "<div>";
        print "<center>";
        print PBDB::TaxonInfo::displayRelatedTaxa($dbt, $taxon_no, $spelling_no, $taxon_name,$is_real_user);
	...
	if($modules{2}) {
        ...
        print '<div align="center">';
        print PBDB::TaxonInfo::displayTaxonHistory($dbt, $taxon_no, $is_real_user);
        print "</div>\n";
#		print "<div align=\"center\"><h3></h3></div>\n";
        if ($in_list && @$in_list) {
            ...
            my $interval_hash = PBDB::TaxonInfo::getIntervalsData($dbt,$data);
            my ($lb,$ub,$minfirst,$max,$min) = PBDB::TaxonInfo::calculateAgeRange($dbt,$data,$interval_hash);
            my $max_no = $max->[0];
            my $min_no = $min->[0];
            if ($max_no || $min_no) {
	...
	if ($modules{3}) {
        ...
        print '<div align="center">';
    	print PBDB::TaxonInfo::displaySynonymyList($dbt, $taxon_no);
        print "</div>\n";
        print "</div>\n";
    ...
    if ($modules{4}) {
        ...
        print '<div align="center">';
#        unless ($quick) {
		    print PBDB::TaxonInfo::displayEcology($dbt,$taxon_no,$in_list);
#        }
        print "</div>\n";
        print "</div>\n";
...
#            print Dumper($row);
            ...
            if ($row->{'parent_spelling_no'} eq '-2') {
                ...
                if ($seen_parent{$row->{'parent_spelling_name'}}) {
                    ...
                    PBDB::Taxon::setOccurrencesTaxonNoByTaxon($dbt,$s->get('authorizer_no'),$id);
                }
                $row->{'parent_no'} = PBDB::TaxonInfo::getOriginalCombination($dbt,$row->{'parent_spelling_no'});
            } else {
                $row->{'parent_no'} = PBDB::TaxonInfo::getOriginalCombination($dbt,$row->{'parent_spelling_no'});
            }
            if ($row->{'child_spelling_no'} eq '-1') {
                ...
                my $t = PBDB::TaxonInfo::getTaxa($dbt,{'taxon_no'=>$row->{'child_spelling_no'}},['taxon_no','taxon_name','taxon_rank','author1init','author1last','author2init','author2last','otherauthors','pubyr','ref_is_authority','reference_no']); 
                if ($t->{'ref_is_authority'} !~ /yes/i && $row->{'author'}) {
                    ...
                    PBDB::Taxon::propagateAuthorityInfo($dbt,$row->{'child_spelling_no'},1);
                }
            }
            ...
            $row->{'child_no'} = PBDB::TaxonInfo::getOriginalCombination($dbt,$row->{'child_spelling_no'});
...
sub validateBlock {
    ...
    my %rank_order = %PBDB::Taxon::rankToNum;
...
sub parseChildName {
    ...
    my @taxa= ();
    if ($no > 0) {
        @taxa= PBDB::TaxonInfo::getTaxa($dbt,{'taxon_no'=>$no},['taxon_no','taxon_name','taxon_rank','author1init','author1last','author2init','author2last','otherauthors','ref_is_authority','pubyr']);
    } else {
        my $restrict_rank = "";
        if ($rank =~ /genus/) {
            $restrict_rank = $rank;
        }
        @taxa = PBDB::TaxonInfo::getTaxa($dbt,{'taxon_name'=>$name,'taxon_rank'=>$restrict_rank},['taxon_no','taxon_name','taxon_rank','author1init','author1last','author2init','author2last','otherauthors','ref_is_authority','pubyr']);
    }
...
#    print "hasCommonAncestor($taxon_no,$parent_no) = ";
    ...
    my $p_hash = PBDB::TaxaCache::getParents($dbt,[$taxon_no]);
...
sub parseParentName {
    ...
    my @nos = PBDB::TaxonInfo::getTaxa($dbt,{'taxon_name'=>$name},['taxon_no','taxon_name','taxon_rank','author1init','author1last','author2init','author2last','otherauthors','ref_is_authority','pubyr']);
    my $taxon_no = 0;
    my @general_errors = ();
    my $homonym_error = undef;
    if (@nos == 0) {
...
sub parseIntervalName {
    my ($dbt,$full_name) = @_;
    my ($eml,$name) = PBDB::TimeLookup::splitInterval($full_name);
    my $no = PBDB::TimeLookup::getIntervalNo($dbt,$eml,$name);
    if ($no) {
        return ($no,[]);
    } else {
...
sub parseAuthorName {
    my $auth = shift;
    my $an = new PBDB::AuthorNames($auth);
...
sub describeTaxon {
    my ($dbt,$t) = @_;
    my $parent = PBDB::TaxaCache::getParent($dbt,$t->{taxon_no});
    my $pub_info = PBDB::Reference::formatShortRef($t);
...
sub listToTrees {
    ...
    my $parents_hash =  PBDB::TaxaCache::getParents($dbt,\@taxon_nos,'array_full');
    my %nodes;
    my %root;
    foreach my $t (@$taxa) {
