# JA 26-31.7, 6.8.07
package PBDB::Cladogram;

#use GD;
use PBDB::Reference;
use Data::Dumper;
use PBDB::Debug qw(dbg);
use PBDB::Constants qw($DATA_DIR $HTML_DIR $WRITE_URL $READ_URL);
...
sub displayCladogramChoiceForm {
    ...
    if (@results == 0) {
        ...
        foreach my $row (@results) {
            my $short_ref = PBDB::Reference::formatShortRef($dbt,$row->{'reference_no'});
...
sub displayCladogramForm {
    ...
    if ($isNewEntry && !$session_ref) {
        ...
        PBDB::displaySearchRefs("Please choose a reference before entering a new cladogram",1);
        return;
    }
    ...
    if ($isResubmit) {
        ...
        $vars = \%vars_copy;
        $vars->{'error_message'} = "<div align=\"center\">".PBDB::Debug::printErrors($errors)."</div><br>";
    } elsif (!$isNewEntry) {
        my $sql = "SELECT * FROM cladograms WHERE cladogram_no=$cladogram_no";
    ...
    my $short_ref = PBDB::Reference::formatShortRef($dbt,$reference_no,'link_id'=>1);
...
#    print Dumper(\@terminal_no)."<br>";
	...
	$caption .= "<br>\n<nobr>Reference: ".PBDB::Reference::formatShortRef($dbt,$cladogram->{reference_no},'no_inits'=>1,'link_id'=>1) . "</nobr><br>\n";
