...
sub get_classification_hash{
    ...
    foreach my $hash_key (@taxon_names_or_nos){
        ...
        # We're using taxon_names as input    
            ...
            my @taxon_nos = PBDB::TaxonInfo::getTaxonNos($dbt,$hash_key);
        ...
        for(my $i=0;$child_no && !$found_parent;$i++) {
            ...
            my ($taxon_rank);
            if ($parent_row) {
                my $taxon= PBDB::TaxonInfo::getMostRecentSpelling($dbt,$child_no,{'reference_no'=>$restrict_to_reference_no});
                ...
                my $taxon = PBDB::TaxonInfo::getTaxa($dbt,{'taxon_no'=>$child_no});
                $taxon_name=$taxon->{'taxon_name'};
                $taxon_rank=$taxon->{'taxon_rank'};
                $status = "";
            }
...
sub getChildren {
    ...
    my $orig_no = PBDB::TaxonInfo::getOriginalCombination($dbt,$taxon_no,$restrict_to_ref);
    my $ss_no = PBDB::TaxonInfo::getSeniorSynonym($dbt,$orig_no,$restrict_to_ref);
...
sub getChildrenRecurse { 
    ...
    foreach my $row (@children) {
        ...
        my $parent_row = PBDB::TaxonInfo::getMostRecentClassification($dbt,$orig_no,{'reference_no'=>$restrict_to_ref});
...
sub createNode {
    ...
    my $taxon = PBDB::TaxonInfo::getMostRecentSpelling($dbt,$orig_no,{'reference_no'=>$restrict_to_ref});
