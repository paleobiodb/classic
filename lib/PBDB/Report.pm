...
sub reportPrintOutfile{
    ...
    $authorizer =~ s/(\s|\.)//g;
    my $reportFileName = $authorizer . "-report.csv";
    PBDB::PBDBUtil::autoCreateDir("$HTML_DIR/public/reports");
...
sub reportQueryDB{
    ...
    my $research_group_sql = PBDB::PBDBUtil::getResearchGroupSQL($dbt,$q->param('research_group'));
		...
		if($q->param('taxon_name')){
            ...
            foreach my $taxon (@taxa) {
                ...
                if (scalar(@taxon_nos) == 0) {
                    ...
                    push @{$self->{'warnings'}}, "The counts are not restricted to '$taxon' because more than one taxon has that name. If this is a problem email the <a href='mailto: admin@paleobiodg.org'>Paleobiology Database Administrator</a>.";
                }
            }
...
sub findMostCommonTaxa	{
	...
	my $filename = PBDB::PBDBUtil::getFilename($username);


	my $csv = Text::CSV_XS->new({'binary'=>1});
	PBDB::PBDBUtil::autoCreateDir("$HTML_DIR/public/taxa");
