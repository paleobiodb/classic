...
sub rebuildCache {
    ...
    if ( @badrows )	{
        ...
        for my $bad ( @badrows )	{
            ...
            $dbh->do($sql);
            my $orig = PBDB::TaxonInfo::getOriginalCombination($dbt, $bad->{'taxon_no'});
            PBDB::TaxonInfo::getMostRecentClassification($dbt,$orig,{'use_synonyms'=>'no'});
        }
    }
...
#    my $range_op = PBDB::TaxonInfo::getMostRecentClassification($dbt,$synonym_no,{'strat_range'=>1});
...
sub updateCache {
    ...
    my $dbh=$dbt->dbh;
    return if (!$child_no);
    $child_no = PBDB::TaxonInfo::getOriginalCombination($dbt,$child_no);
...
#  there is no check on the record's created time)
    ...
    my $mrpo = PBDB::TaxonInfo::getMostRecentClassification($dbt,$child_no,{'recompute'=>'yes'});
    ...
    my @spellings = PBDB::TaxonInfo::getAllSpellings($dbt,$child_no);
...
#    my @results = @{$dbt->getData($sql)};
    ...
    # New most recent opinion
    my $spelling = PBDB::TaxonInfo::getMostRecentSpelling($dbt,$child_no);
    my $spelling_no = $spelling->{'taxon_no'};
       
    # Change the most current spelling_no and max and min
    my $range_op = PBDB::TaxonInfo::getMostRecentClassification($dbt,$child_no,{'strat_range'=>1});
    my ($max_interval_no,$min_interval_no) = (0,0);
    if ($range_op) {
        $max_interval_no = $range_op->{'max_interval_no'};
        $min_interval_no = $range_op->{'min_interval_no'};
	...
	my $senior_synonym_no = PBDB::TaxonInfo::getSeniorSynonym($dbt,$child_no);
	my $senior_synonym_spelling = PBDB::TaxonInfo::getMostRecentSpelling($dbt,$senior_synonym_no);
	$senior_synonym_no = $senior_synonym_spelling->{'taxon_no'} || $senior_synonym_no;
    ...
    if ($new_parent_no) {
        ...
        my $parent_spelling = PBDB::TaxonInfo::getMostRecentSpelling($dbt,$new_parent_no);
...
sub moveChildren {
    ...
    if ($parent_no && $p_row->{'lft'} > $c_row->{'lft'} && $p_row->{'rgt'} < $c_row->{'rgt'}) {
        ...
        my $popinion = PBDB::TaxonInfo::getMostRecentClassification($dbt,$parent_no,{'use_synonyms'=>'no'});
...
sub getChildren {
    ...
    return undef unless defined $taxon_no && $taxon_no > 0;
...
sub getParents {
	...
	if ( $taxon_no =~ /^(\d+)/ )
	{
	    ...
	    my $sql = "SELECT a.taxon_no,a.taxon_name,a.taxon_rank,a.common_name,a.extant, IF (a.pubyr IS NOT NULL AND a.pubyr != '' AND a.pubyr != '0000', a.pubyr, IF (a.ref_is_authority='YES', r.pubyr, '')) pubyr FROM $TAXA_TREE_CACHE as t join $TAXA_TREE_CACHE as base on t.lft <= base.lft and t.rgt >= base.rgt, authorities a,refs r WHERE a.taxon_no=t.taxon_no AND base.taxon_no=$taxon_no AND base.lft > 0 AND base.rgt >= base.lft AND t.taxon_no = t.synonym_no AND a.reference_no=r.reference_no ORDER BY t.lft DESC";
...
sub getParent {
    ...
    my $sql = "SELECT t.taxon_no FROM $TAXA_TREE_CACHE as t JOIN $TAXA_TREE_CACHE as base on t.lft <= base.lft and t.rgt >= base.rgt WHERE base.taxon_no=$taxon_no AND t.taxon_no = t.synonym_no $rank_sql AND base.lft > 0 AND base.rgt >= base.lft ORDER BY t.lft DESC LIMIT 1";
...
sub getMetaData {
    ...
    my $last_op = PBDB::TaxonInfo::getMostRecentClassification($dbt,$orig_no);
    ...
    if ($synonym_no != $spelling_no || ($last_op && $last_op->{'status'} !~ /belongs to/)) {
        if ($last_op->{'status'} =~ /nomen/) {
            my $last_parent_op = PBDB::TaxonInfo::getMostRecentClassification($dbt,$orig_no,{'exclude_nomen'=>1}); 
