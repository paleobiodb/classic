...
/opt/local/lib/mariadb-10.2/bin/mysqldump -u pbdb_backup -p$MYSQL_BACKUP_PASSWD --opt pbdb | gzip > "$MYSQL_BACKUP_DIR/$OF.gz"

echo "Backing up core tables to $MYSQL_BACKUP_DIR/$CORE_DUMP.gz"
/opt/local/lib/mariadb-10.2/bin/mysqldump -u pbdb_backup -p$MYSQL_BACKUP_PASSWD --opt pbdb $CORE_TABLES | gzip > "$MYSQL_BACKUP_DIR/$CORE_DUMP.gz"

echo "Backing up wing tables to $MYSQL_BACKUP_DIR/$OFW.gz"
/opt/local/lib/mariadb-10.2/bin/mysqldump -u pbdb_backup -p$MYSQL_BACKUP_PASSWD --opt pbdb_wing | gzip > "$MYSQL_BACKUP_DIR/$OFW.gz"
