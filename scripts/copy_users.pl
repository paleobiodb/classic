...
foreach my $pbdb_user ( @$PBDB_USERLIST )
{
    ...
    my $email = decode_utf8( $pbdb_user->{email} ) || '';
    ...
    # Grab all relevant fields from the PBDB user record.  Each of the following might
    # contain non-ASCII text, so must be decoded from UTF-8.
    ...
    my $institution = decode_utf8( $pbdb_user->{institution} ) || 'none given';
    ...
    # PBDB database, add an AuthEnt object to the Wing database.
    ...
    print LOGFILE "    Authorizers: $authorizer_list\n";
    print LOGFILE "\n";
}
...
sub read_authent_table {
    ...
    my $sql = " SELECT distinct authorizer_no, enterer_no 
	...
	$IS_ENTERER_FOR{$enterer_no}{$authorizer_no} = 1;
    }
}
